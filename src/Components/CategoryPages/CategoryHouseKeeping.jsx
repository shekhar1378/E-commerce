...
function CategoryHouseKeeping() {
  const filteredItems = data.filter((item) => item.type === "HouseKeeping");

  if (filteredItems.length === 0) {
    return <div className="h-full m-40 flex items-center justify-center text-xl">
        <h1>No Product found.</h1>
        {/* <img   src="./src/assets/notfound.jpg" alt="notfound" /> */}
    </div>;
  }
  return (
    <>
        ...
        <div className=" w-[70%] gap-2 grid grid-cols-4 grid-row-auto ">
          {filteredItems.map((item) => (
            <div key={item.id} className="product normal">
              <Link to={`/categories/KitchenTable/${item.id}`}>
...
export default CategoryHouseKeeping;
